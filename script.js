$(document).ready(function() {
  ...
  // Show scoreboard after completing or failing level
  function showScoreboard(won) {
    ...
    if (won) {
      // Show next level and restart buttons when won
      if (currentLevel < 3) {
        $("#next-level").removeClass("hidden");
      } else {
        $("#next-level").addClass("hidden"); // No next level after Level 3
      }
      $("#restart").removeClass("hidden");
      $("#try-again").addClass("hidden");
    } else {
      // Only show restart button with "Try Again" message when time ran out
  ...
  $("#next-level").click(function() {
    ...
    $("#scoreboard").addClass("hidden"); // Hide scoreboard when starting next level
    resetGame();
